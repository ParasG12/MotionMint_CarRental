...
namespace CarVariantMVC.Controllers
{
    public class AccountController : Controller
    {
        ...
        //            // For example, set the user session, and redirect to the appropriate dashboard
        //            Response.Cookies.Append("Username", user.Username, new CookieOptions { Expires = DateTimeOffset.UtcNow.AddDays(30) });
        //            Response.Cookies.Append("Password", user.PasswordHash, new CookieOptions { Expires = DateTimeOffset.UtcNow.AddDays(30) }); // Store password hash only if necessary
